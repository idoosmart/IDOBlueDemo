...
/**
 启动数据迁移,传入迁移的数据是否来自云端,迁移完数据后会删除废弃目录及废弃的文件,旧的数据库将存放新的文件夹目录中
  ---------------------------------------------->>>>>-------------------------------------------------------
 Start data migration, whether the incoming data is from the cloud, delete the obsolete directory and discarded files after the data is migrated, and the old database will be stored in the new folder directory.
 */
...
/**
 ...
 ⚠️如果不传入目录集合，除了sdk新创建的目录不删除之外其他都会删除，所以根据开发需求自行传入目录名字集合
   ---------------------------------------------->>>>>-------------------------------------------------------
 Data migration is completed direNames is passed to the collection of directory names that cannot be deleted. In the other directories in Documents, all the data will be deleted after the migration.
   ⚠️If you do not pass the directory collection, except for the newly created directory of sdk, it will be deleted, so you will pass the directory name collection according to the development requirements.
 */
