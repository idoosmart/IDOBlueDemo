...
#if DEBUG
        registrationServices().outputSdkLog!(true).outputProtocolLog!(true).startScanBule!{
            (model) in
            if IDOBluetoothEngine.shareInstance()?.peripheralEngine.isBind == true {
                IDOBluetoothManager.startScan()
            }
        }
#else
        registrationServices().outputSdkLog!(false).outputProtocolLog!(false).startScanBule!{
            (model) in
            if IDOBluetoothEngine.shareInstance()?.peripheralEngine.isBind == true {
                IDOBluetoothManager.startScan()
            }
