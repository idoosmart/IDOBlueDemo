...
#define logNormalFont(fontSize) [UIFont systemFontOfSize:fontSize]
// RGB颜色
...
#define logConsoleBoardOriginWidth kScreenWidth / MaxConsoleWidthSCale
#define logConsoleBoardOriginHeight kScreenHeight / MaxConsoleHeightSCale
...
@interface IDOConsoleBoard()<UITextViewDelegate>
...
@implementation IDOConsoleBoard

+ (instancetype)borad {
    static IDOConsoleBoard* _board;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _board = [[IDOConsoleBoard alloc] init];
    });
    return _board;
}
...
- (void)addLog:(NSNotification *)notice {
    ...
    [totalText addAttribute:NSFontAttributeName value:self.textView.font range:NSMakeRange(0, totalText.length)];
    [totalText addAttribute:NSForegroundColorAttributeName value:self.textView.textColor range:NSMakeRange(0, totalText.length)];
...
- (void)createBoard {
    ...
    _contentView.frame = CGRectMake(0,kScreenHeight/2,logConsoleBoardOriginWidth,logConsoleBoardOriginHeight);
    _contentView.clipsToBounds = NO;
    _toolView =  [[UIView alloc] init];
    _toolView.backgroundColor = RGBA(230,230,230,0.9);
    
    [KEYWINDOW addSubview:_toolView];
    ...
    [_closeBtn setImage:IMAGE(@"idoLogRes.bundle/ido_icon_welfaregoods_detail_close") forState:UIControlStateNormal];
    ...
    [clean setImage:[IMAGE(@"idoLogRes.bundle/ido_btn_coinDeail_timeDelete") imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate] forState:UIControlStateNormal];
   ...
   // [search setTitle:@"search" forState:UIControlStateNormal];
    ...
    [search setImage:[IMAGE(@"idoLogRes.bundle/ido_log_search_icon") imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate] forState:UIControlStateNormal];
    ...
    _searchTextField.font = logNormalFont(14);
    ...
    done.titleLabel.font = logNormalFont(12);
    ...
    _textView.font = logNormalFont(10);
...
#pragma mark - 显示
- (void)show {
    if([KEYWINDOW.subviews containsObject:_contentView]) {
...
- (void)down {
    ...
    self.textView.font = logNormalFont(pointSize);
}
...
- (void)searchWord:(NSString *)searchText {
    ...
    if (searchText.length == 0) {
        self.searchsResult = nil;
        self.currentSearchIndex = 0;
        [attStr addAttribute:NSFontAttributeName value:self.textView.font range:NSMakeRange(0, attStr.length)];
        [attStr addAttribute:NSForegroundColorAttributeName value:self.textView.textColor range:NSMakeRange(0, attStr.length)];
...
-(void)dragArrow:(UIPanGestureRecognizer *)rec {
    ...
    if (rec.state == UIGestureRecognizerStateBegan) {
        ...
        CGFloat width =  MIN(MAX(self.contentView.frame.size.width + point.x, MinConsoleWidth), logConsoleBoardOriginWidth * MaxConsoleWidthSCale);
        CGFloat height = MIN(MAX(self.contentView.frame.size.height + point.y, MinConsoleHeight), logConsoleBoardOriginHeight * MaxConsoleHeightSCale);
...
- (void)handlePinch:(UIPinchGestureRecognizer *)pinchGestureRecognizer
{
    ...
    if (pinchGestureRecognizer.state == UIGestureRecognizerStateBegan || pinchGestureRecognizer.state == UIGestureRecognizerStateChanged) {
        ...
        view.frame = CGRectMake(0, 0, MIN(MAX(view.frame.size.width * scale, MinConsoleWidth), logConsoleBoardOriginWidth * MaxConsoleWidthSCale)  , MIN(MAX(view.frame.size.height * scale, MinConsoleHeight), logConsoleBoardOriginHeight * MaxConsoleHeightSCale));
        view.center = center;
