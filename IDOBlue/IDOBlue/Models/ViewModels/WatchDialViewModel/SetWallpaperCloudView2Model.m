...
- (void)getCellModels
{
    ...
    model1.data    = @[lang(@"Make And Install wallpaper cloud dial file")];
...
- (void)startMakingWapperFromCloud:(FuncViewController*)funcVc{
    ...
    [PhotoDialManager makeAndInstallTheDialWtith:self.wallpaperCloudManager bgImage:select previewSetImage:previewImage firmwareName:self.fileName zipName:self.fileName callback:^(WallpaperWCInstallFaceLog status, NSString * _Nonnull wallpaperZipPath,int progress) {
        ...
        if (status == IDOWallpaperWCInstallFaceLogUpgradeSucc) {
            ...
            [funcVc showToastWithText:lang(@"install dial file success")];
        }else if(status == IDOWallpaperWCInstallFaceLogUpgradIng){
            [funcVc showSyncProgress:progress/100.0f];

        }else{
            [funcVc showToastWithText:lang(@"install dial file failed")];
        }
    }];
