...
@interface GetMtuInfoViewModel()
@property (nonatomic,strong) UITextView * textView;
@property (nonatomic,copy)void(^textViewCallback)(UITextView * textView);
@property (nonatomic,copy)void(^buttconCallback)(UIViewController * viewController,UITableViewCell * tableViewCell);
@end

@implementation GetMtuInfoViewModel
- (instancetype)init
{
    self = [super init];
    if (self) {
...
- (void)getButtonCallback
{
    ...
    self.buttconCallback = ^(UIViewController *viewController, UITableViewCell *tableViewCell) {
        ...
        [funcVC showLoadingWithMessage:[NSString stringWithFormat:@"%@...",lang(@"get blue mtu info")]];
        [IDOFoundationCommand getMtuInfoCommand:^(int errorCode, IDOGetMtuBlueInfoModel * _Nullable data) {
            if (errorCode == 0) {
               [funcVC showToastWithText:lang(@"get blue mtu info success")];
               strongSelf.textView.text = [NSString stringWithFormat:@"%@",data.dicFromObject];
            }else if (errorCode == 6) {
               [funcVC showToastWithText:lang(@"feature is not supported on the current device")];
            }else {
               [funcVC showToastWithText:lang(@"get blue mtu info failed")];
            }
        }];
    };
}
...
- (void)getCellModels
{
    ...
    model.data = @[lang(@"get blue mtu info")];
