...
- (instancetype)init {
    ...
    _engine = [IDOEngine sharedEngine];
    return self;
}
...
- (void)setupConfig:(void(^)(XMConfig *config))block {
    ...
    if (config.commonUrlSuffix) {
        self.commonUrlSuffix = config.commonUrlSuffix;
    }
...
- (NSString *)sendRequest:(XMRequestConfigBlock)configBlock
               onProgress:(nullable XMProgressBlock)progressBlock
                ...
                onFailure:(nullable XMFailureBlock)failureBlock
               onFinished:(nullable XMFinishedBlock)finishedBlock {
    IDORequest *request = [IDORequest request];
...
- (NSString *)sendBatchRequest:(XMBatchRequestConfigBlock)configBlock
    ...
    if (batchRequest.requestArray.count > 0) {
        ...
        for (IDORequest *request in batchRequest.requestArray) {
...
- (void)cancelRequest:(NSString *)identifier
    ...
    if ([identifier hasPrefix:@"BC"]) {
        ...
        if ([request isKindOfClass:[XMBatchRequest class]]) {
            ...
            if (batchRequest.requestArray.count > 0) {
                for (IDORequest *rq in batchRequest.requestArray) {
...
+ (void)setupConfig:(void(^)(XMConfig *config))block {
    [[IDOCenter defaultCenter] setupConfig:block];
}

+ (void)setRequestProcessBlock:(XMCenterRequestProcessBlock)block {
    [[IDOCenter defaultCenter] setRequestProcessBlock:block];
}

+ (void)setResponseProcessBlock:(XMCenterResponseProcessBlock)block {
    [[IDOCenter defaultCenter] setResponseProcessBlock:block];
}

+ (void)setErrorProcessBlock:(XMCenterErrorProcessBlock)block {
    [[IDOCenter defaultCenter] setErrorProcessBlock:block];
}

+ (void)setGeneralHeaderValue:(NSString *)value forField:(NSString *)field {
    [[IDOCenter defaultCenter].generalHeaders setValue:value forKey:field];
}

+ (void)setGeneralParameterValue:(id)value forKey:(NSString *)key {
    [[IDOCenter defaultCenter].generalParameters setValue:value forKey:key];
}

#pragma mark -

+ (NSString *)sendRequest:(XMRequestConfigBlock)configBlock {
    return [[IDOCenter defaultCenter] sendRequest:configBlock onProgress:nil onSuccess:nil onFailure:nil onFinished:nil];
}

+ (NSString *)sendRequest:(XMRequestConfigBlock)configBlock
                onSuccess:(nullable XMSuccessBlock)successBlock {
    return [[IDOCenter defaultCenter] sendRequest:configBlock onProgress:nil onSuccess:successBlock onFailure:nil onFinished:nil];
}

+ (NSString *)sendRequest:(XMRequestConfigBlock)configBlock
                onFailure:(nullable XMFailureBlock)failureBlock {
    return [[IDOCenter defaultCenter] sendRequest:configBlock onProgress:nil onSuccess:nil onFailure:failureBlock onFinished:nil];
}

+ (NSString *)sendRequest:(XMRequestConfigBlock)configBlock
               onFinished:(nullable XMFinishedBlock)finishedBlock {
    return [[IDOCenter defaultCenter] sendRequest:configBlock onProgress:nil onSuccess:nil onFailure:nil onFinished:finishedBlock];
}

+ (NSString *)sendRequest:(XMRequestConfigBlock)configBlock
                onSuccess:(nullable XMSuccessBlock)successBlock
                onFailure:(nullable XMFailureBlock)failureBlock {
    return [[IDOCenter defaultCenter] sendRequest:configBlock onProgress:nil onSuccess:successBlock onFailure:failureBlock onFinished:nil];
}
...
+ (NSString *)sendRequest:(XMRequestConfigBlock)configBlock
                ...
                onFailure:(nullable XMFailureBlock)failureBlock
               onFinished:(nullable XMFinishedBlock)finishedBlock {
    return [[IDOCenter defaultCenter] sendRequest:configBlock onProgress:nil onSuccess:successBlock onFailure:failureBlock onFinished:finishedBlock];
}
...
+ (NSString *)sendRequest:(XMRequestConfigBlock)configBlock
               onProgress:(nullable XMProgressBlock)progressBlock
                onSuccess:(nullable XMSuccessBlock)successBlock
                onFailure:(nullable XMFailureBlock)failureBlock {
    return [[IDOCenter defaultCenter] sendRequest:configBlock onProgress:progressBlock onSuccess:successBlock onFailure:failureBlock onFinished:nil];
}
...
+ (NSString *)sendRequest:(XMRequestConfigBlock)configBlock
               onProgress:(nullable XMProgressBlock)progressBlock
                ...
                onFailure:(nullable XMFailureBlock)failureBlock
               onFinished:(nullable XMFinishedBlock)finishedBlock {
    return [[IDOCenter defaultCenter] sendRequest:configBlock onProgress:progressBlock onSuccess:successBlock onFailure:failureBlock onFinished:finishedBlock];
}
...
+ (NSString *)sendBatchRequest:(XMBatchRequestConfigBlock)configBlock
                     ...
                     onFailure:(nullable XMBCFailureBlock)failureBlock
                    onFinished:(nullable XMBCFinishedBlock)finishedBlock {
    return [[IDOCenter defaultCenter] sendBatchRequest:configBlock onSuccess:successBlock onFailure:failureBlock onFinished:finishedBlock];
}
...
+ (NSString *)sendChainRequest:(XMChainRequestConfigBlock)configBlock
                     ...
                     onFailure:(nullable XMBCFailureBlock)failureBlock
                    onFinished:(nullable XMBCFinishedBlock)finishedBlock {
    return [[IDOCenter defaultCenter] sendChainRequest:configBlock onSuccess:successBlock onFailure:failureBlock onFinished:finishedBlock];
}

#pragma mark -

+ (void)cancelRequest:(NSString *)identifier {
    [[IDOCenter defaultCenter] cancelRequest:identifier onCancel:nil];
}

+ (void)cancelRequest:(NSString *)identifier
             onCancel:(nullable XMCancelBlock)cancelBlock {
    [[IDOCenter defaultCenter] cancelRequest:identifier onCancel:cancelBlock];
}

+ (nullable id)getRequest:(NSString *)identifier {
    return [[IDOCenter defaultCenter] getRequest:identifier];
}

+ (BOOL)isNetworkReachable {
    return [[IDOCenter defaultCenter] isNetworkReachable];
}

+ (XMNetworkConnectionType)networkConnectionType {
    return [[IDOCenter defaultCenter] networkConnectionType];
}

#pragma mark -

+ (void)addSSLPinningURL:(NSString *)url {
    [[IDOCenter defaultCenter].engine addSSLPinningURL:url];
}

+ (void)addSSLPinningCert:(NSData *)cert {
    [[IDOCenter defaultCenter].engine addSSLPinningCert:cert];
}

+ (void)addTwowayAuthenticationPKCS12:(NSData *)p12 keyPassword:(NSString *)password {
    [[IDOCenter defaultCenter].engine addTwowayAuthenticationPKCS12:p12 keyPassword:password];
}
...
- (void)xm_processRequest:(IDORequest *)request
    ...
    //优先判断是否动态响应了公共参数
    if ([IDONetworkManager sharedManager].delegate&&[[IDONetworkManager sharedManager].delegate respondsToSelector:@selector(mutebleCommonUrlSuffix)]) {
        
        NSString * mutebleCommonUrlSuffix = [[IDONetworkManager sharedManager].delegate mutebleCommonUrlSuffix];
        
        if (mutebleCommonUrlSuffix) {
            if ([request.url containsString:@"?"]) {
                request.url = [NSString stringWithFormat:@"%@&%@",request.url,mutebleCommonUrlSuffix];
            }else{
                request.url = [NSString stringWithFormat:@"%@?%@",request.url,mutebleCommonUrlSuffix];
            }
        }
        ...
        if ([IDOCenter defaultCenter].commonUrlSuffix) {
            request.url = [NSString stringWithFormat:@"%@?%@",request.url,[IDOCenter defaultCenter].commonUrlSuffix];
        }
    }
...
- (void)xm_sendRequest:(IDORequest *)request {
...
- (void)xm_successWithResponse:(id)responseObject forRequest:(IDORequest *)request {
...
- (void)xm_execureSuccessBlockWithResponse:(id)responseObject forRequest:(IDORequest *)request {
...
- (void)xm_failureWithError:(NSError *)error forRequest:(IDORequest *)request {
...
- (void)xm_execureFailureBlockWithError:(NSError *)error forRequest:(IDORequest *)request {
