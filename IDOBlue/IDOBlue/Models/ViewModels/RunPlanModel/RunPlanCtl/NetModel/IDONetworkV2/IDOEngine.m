...
@property (nonatomic, strong) IDORequest *bindedRequest;

@end

@implementation NSURLSessionTask (XMRequest)

- (IDORequest *)bindedRequest {
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setBindedRequest:(IDORequest *)bindedRequest {
...
- (void)sendRequest:(IDORequest *)request completionHandler:(XMCompletionHandler)completionHandler {
...
- (IDORequest *)cancelRequestByIdentifier:(NSString *)identifier {
    ...
    __block IDORequest *request = nil;
    if (tasks.count > 0) {
...
- (IDORequest *)getRequestByIdentifier:(NSString *)identifier {
    if (identifier.length == 0) return nil;
...
- (void)xm_dataTaskWithRequest:(IDORequest *)request
...
- (void)xm_uploadTaskWithRequest:(IDORequest *)request
               completionHandler:(XMCompletionHandler)completionHandler {
...
- (void)xm_downloadTaskWithRequest:(IDORequest *)request
                 completionHandler:(XMCompletionHandler)completionHandler {
...
- (void)xm_processURLRequest:(NSMutableURLRequest *)urlRequest byXMRequest:(IDORequest *)request {
...
- (void)xm_setIdentifierForReqeust:(IDORequest *)request
...
- (AFURLSessionManager *)xm_getSessionManager:(IDORequest *)request {
...
- (AFHTTPRequestSerializer *)xm_getRequestSerializer:(IDORequest *)request {
...
- (AFHTTPResponseSerializer *)xm_getResponseSerializer:(IDORequest *)request {
    if (request.responseSerializerType == kXMResponseSerializerRAW) {
...
- (AFJSONResponseSerializer *)afJSONResponseSerializer {
    if (!_afJSONResponseSerializer) {
        ...
        _afJSONResponseSerializer.removesKeysWithNullValues = YES;
