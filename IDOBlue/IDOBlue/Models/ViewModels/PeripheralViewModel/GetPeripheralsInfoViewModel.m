...
@interface GetPeripheralsInfoViewModel()<IDOPeripheralsManagerDelegate>
@property (nonatomic,strong) UITextView * textView;
@property (nonatomic,copy)void(^textViewCallback)(UITextView * textView);
@property (nonatomic,copy)void(^buttconCallback)(UIViewController * viewController,UITableViewCell * tableViewCell);
@end

@implementation GetPeripheralsInfoViewModel
...
- (void)getButtonCallback
{
    self.buttconCallback = ^(UIViewController *viewController, UITableViewCell *tableViewCell) {
       ...
       //获取
       NSInteger error = [IDOPeripheralsManager getPeripheralsInfoData];
       NSLog(@"%d",error);
    };
}
...
- (void)getCellModels
{
    ...
    model.data = @[lang(@"get Peripherals Info")];
...
//信息回调数据
-(void)getPeripheralsInfoData:(IDOPeripheralsInfoModel *)model errorCode:(int)errorCode{
    NSLog(@"------->%d",errorCode);
    FuncViewController * funcVC = (FuncViewController *)[IDODemoUtility getCurrentVC];
