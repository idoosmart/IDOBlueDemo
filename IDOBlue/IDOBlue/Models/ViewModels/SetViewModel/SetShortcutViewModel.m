...
- (void)getCellModels
{
    ...
    model2.data = @[lang(@"fast into the photo control")];
    ...
    model3.data = @[lang(@"fast into sport mode")];
    ...
    model4.data = @[lang(@"fast into no disturb mode")];
    ...
    model6.data = @[lang(@"set shortcut mode")];
...
- (void)getButtonCallback
{
    ...
    self.buttconCallback = ^(UIViewController *viewController, UITableViewCell *tableViewCell) {
        ...
        [funcVC showLoadingWithMessage:[NSString stringWithFormat:@"%@...",lang(@"set shortcut mode")] ];
        [IDOFoundationCommand setShortcutCommand:strongSelf.shortcutModel
                                        callback:^(int errorCode) {
            if(errorCode == 0) {
                [funcVC showToastWithText:lang(@"set shortcut mode success") ];
            }else if (errorCode == 6) {
                [funcVC showToastWithText:lang(@"feature is not supported on the current device")];
            }else {
                [funcVC showToastWithText:lang(@"set shortcut mode failed") ];
            }
        }];
    };
}
