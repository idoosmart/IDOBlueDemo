//
//  Setv3ScientificSleepViewModel.m
//  IDOBlue
//
...
@implementation Setv3ScientificSleepViewModel
        ...
        if (indexPath.row == 1) {
            ...
            funcVC.pickerView.pickerViewCallback = ^(NSString *selectStr) {
                ...
                if (twoCell.textField1 == textField) {
                    strongSelf.sleepModel.startHour    = [selectStr integerValue];
                }else {
                    strongSelf.sleepModel.startMinute  = [selectStr integerValue];
                }
                textFieldModel.data = @[@(strongSelf.sleepModel.startHour),@(strongSelf.sleepModel.startMinute)];
            ...
            funcVC.pickerView.pickerViewCallback = ^(NSString *selectStr) {
                textField.text = selectStr;
                if (twoCell.textField1 == textField) {
                    strongSelf.sleepModel.endHour    = [selectStr integerValue];
                }else {
                    strongSelf.sleepModel.endMinute  = [selectStr integerValue];
                }
                textFieldModel.data = @[@(strongSelf.sleepModel.endHour),@(strongSelf.sleepModel.endMinute)];
                [[(FuncViewController *)viewController tableView] reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationNone];
            };
        }
    };
...
- (void)getButtonCallback
{
    ...
    self.buttconCallback = ^(UIViewController *viewController, UITableViewCell *tableViewCell) {
        ...
        [IDOFoundationCommand setScientificSleepCommand:strongSelf.sleepModel
...
- (void)getSwitchCallback
{
    ...
    self.switchCallback = ^(UIViewController *viewController, UISwitch *onSwitch, UITableViewCell *tableViewCell) {
        ...
        if (indexPath.row == 0) {
            ...
            strongSelf.sleepModel.mode = onSwitch.isOn;
            switchCellModel.data = @[@(strongSelf.sleepModel.mode)];
        }
    };
}
...
- (void)getCellModels
{
    ...
    model1.titleStr = lang(@"set sleep switch model");
    model1.data = @[@(self.sleepModel.mode)];
