...
@interface SetContentViewModel()
@property (nonatomic,copy)void(^textFeildCallback)(UIViewController * viewController,UITextField * textField,UITableViewCell * tableViewCell);
@property (nonatomic,copy)void(^buttconCallback)(UIViewController * viewController,UITableViewCell * tableViewCell);
@property (nonatomic,strong) UITextField * textField;
@end

@implementation SetContentViewModel
- (instancetype)init
{
    self = [super init];
    if (self) {
...
- (void)getCellModels
{
    ...
    model1.titleStr = [NSString stringWithFormat:@"%@ :",lang(@"set message push")];
    model1.data = @[@""];
    ...
    model3.data = @[lang(@"set message push")];
    ...
    self.buttconCallback = ^(UIViewController *viewController, UITableViewCell *tableViewCell) {
        ...
        [IDOFoundationCommand setContentCommand:strongSelf.textField.text callback:^(int errorCode) {
            if(errorCode == 0) {
               ...
               [funcVC showToastWithText:lang(@"set message push failed")];
            }
        }];
    };
}
