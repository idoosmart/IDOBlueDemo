 ...
 */
typedef void(^DHDeviceScanFinishedBlock)( NSString * _Nullable scanString);

/**
 监听环境光感的回调
 @param brightness 亮度值
 */
typedef void(^DHDeviceMonitorLightBlock)( float brightness);

@interface DeviceNativeScanTool : NSObject

/**
 扫描出结果后的回调 ，注意循环引用的问题
 */
@property (nonatomic, copy) DHDeviceScanFinishedBlock _Nullable scanFinishedBlock;

/**
 监听环境光感的回调,如果 != nil 表示开启监测环境亮度功能
 */
@property (nonatomic, copy) DHDeviceMonitorLightBlock _Nullable monitorLightBlock;
